...
// ============================================================================
// 非阻塞 + 跨平台文件选择器模块 (隔离 RFD 以修复 Android 编译)
// ============================================================================
...
mod file_picker {
    ...
    pub fn save_scenario(tx: Sender<AppCommand>, _json_data: String) {
        let _ = tx.send(AppCommand::Log("[系统] 安卓端暂不支持原生保存弹窗".into()));
    }
    pub fn load_scenario(tx: Sender<AppCommand>) {
        let _ = tx.send(AppCommand::Log("[系统] 安卓端暂不支持原生加载弹窗".into()));
    }
    pub fn pick_spine(tx: Sender<AppCommand>, _slot: usize) {
        let _ = tx.send(AppCommand::Log("[系统] 安卓端暂不支持原生文件选择".into()));
    }
    pub fn pick_bg(tx: Sender<AppCommand>) {
        let _ = tx.send(AppCommand::Log("[系统] 安卓端暂不支持原生文件选择".into()));
    }
...
enum AppCommand {
    ...
    Log(String), // 日志记录
    ScenarioLoaded(Scenario), // 后台线程反向传递已加载/解析完毕的剧本
}
...
pub struct SpineObject {
    ...
    // 🌟 修正：rusty_spine3.8 的设计是全部采用 Arc 引用计数，
    // 所以不需要复杂的生命周期，直接持用结构体即可！
    skeleton: Skeleton,               
...
impl SpineObject {
    ...
    fn load_async_no_gpu(path_str: &str) -> Result<(Self, egui::ColorImage, String, Vec<String>), String> {
        ...
        let raw_pixels = rgba.into_raw(); // 获取 Vec<u8>
    ...
    fn push_to_mesh(&self, mesh: &mut Mesh, w_v: &[f32], uvs: &[f32], tris: &[u16], slot: &Slot, att_c: rusty_spine::Color) {
        let s_c = slot.color(); // 插槽颜色
        
        // 顶点颜色必须预乘 Alpha (Premultiplied Alpha)
        let a = s_c.a * att_c.a;
        let r = s_c.r * att_c.r * a;
        let g = s_c.g * att_c.g * a;
        let b = s_c.b * att_c.b * a;

        // 处理 Additive (加法) 混合模式
        let final_a = match slot.data().blend_mode() {
            rusty_spine::BlendMode::Additive => 0.0,
            _ => a,
        };

        // 使用 clamp 防止浮点数精度问题导致的溢出
        ...
        for i in 0..count {
            let pos = Pos2::new(
                ...
                -w_v[i*2+1] * self.scale + self.position.y // Y轴翻转
            );
        ...
        while let Ok(cmd) = self.rx.try_recv() {
            match cmd {
                ...
                AppCommand::ScenarioLoaded(s) => {
                    ...
                    self.sync_scene_to_ui();
                    self.visible_count = self.target_chars.len(); // 展示全部文字
...
fn draw_creator_panel(ctx: &egui::Context, app: &mut AefrApp) {
    ...
    egui::Window::new("创作者面板 - GNU:AEFR")
        ...
        .show(ctx, |ui| {
            ...
            // 🌟 全异步化的保存/重载逻辑，通过分离的 file_picker 模块跨平台调用
            ui.horizontal(|ui| {
            ...
            ui.horizontal(|ui| {
                ...
                if ui.add(egui::Button::new("🗑 立绘移除").fill(Color32::from_rgb(150, 40, 40))).clicked() {
...
impl SpineObject {
    ...
    fn push_to_mesh(&self, mesh: &mut Mesh, w_v: &[f32], uvs: &[f32], tris: &[u16], slot: &Slot, att_c: rusty_spine::Color) {
        ...
        // 🌟 修复点 1：顶点颜色必须预乘 Alpha (Premultiplied Alpha)
        // 之前没有乘 a，导致半透明区域被 egui 识别为超亮 HDR 颜色，变成实心白块
        let a = s_c.a * att_c.a;
        let r = s_c.r * att_c.r * a;
        let g = s_c.g * att_c.g * a;
        let b = s_c.b * att_c.b * a;

        // 🌟 修复点 2：处理 Additive (加法) 混合模式
        // egui 默认使用 PMA 混合: Out = Src + Dst * (1 - SrcAlpha)
        // 如果我们将 SrcAlpha 设为 0，公式就变成了 Out = Src + Dst，完美实现 Additive 混合！
        let final_a = match slot.data().blend_mode() {
            rusty_spine::BlendMode::Additive => 0.0,
            _ => a,
        };
